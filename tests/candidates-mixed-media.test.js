...
const modalBody = {};
root.parentElement = modalBody;
globalThis.getComputedStyle = element => ({ pointerEvents: element === modalBody ? "none" : "auto" });
assert.equal(plugin.candidates.isInteractiveElement(image), true, "dialog children can restore pointer-events under the modal body");
modalBody.inert = true;
assert.equal(plugin.candidates.isInteractiveElement(image), false, "real inert ancestors must still block interaction");
modalBody.inert = false;
globalThis.getComputedStyle = () => ({ pointerEvents: "none" });
assert.equal(plugin.candidates.isInteractiveElement(image), false, "a noninteractive row is still rejected");
delete globalThis.getComputedStyle;
delete root.parentElement;
let escapes = 0;
const dialogEditor = { closest: () => ({}) };
plugin.editor = { findEditor: () => dialogEditor };
plugin.canvas = { cleanupPicker: () => true, pickerIsOpen: () => true };
document.activeElement = { dispatchEvent: () => { escapes++; } };
plugin.candidates.closePicker();
plugin.canvas.cleanupPicker = () => false;
plugin.canvas.pickerIsOpen = () => false;
plugin.candidates.closePicker();
assert.equal(escapes, 0, "owned-query cleanup or a closed picker must not send Escape into the generator dialog");
delete plugin.canvas;
delete plugin.editor;
console.log("✓ expanded canvas menus inherit clickable dialog styles and cleanup never closes the generator with an extra Escape");
let revealed = false;
