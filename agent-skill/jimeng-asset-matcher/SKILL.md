...
内置 v0.3.19 的完整运行源码，无需编译、API Key、MCP 或 npm 依赖。扩展提供“自动上传 → 自动匹配”两步流程，普通创作页与无限画布分别处理当前输入框，并随网页浅色 / 深色模式自动换色。
...
5. 验证扩展卡片名称“即梦素材一键匹配”和版本 `0.3.19`；保存提示词、刷新即梦页面并点击视频提示词框，确认按钮出现。只有实际看见这些结果才能报告浏览器端已启用。`browser_status: manual_load_required` 仅表示文件准备完成。
